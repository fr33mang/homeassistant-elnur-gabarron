...
class ElnurSocketIOCoordinator(DataUpdateCoordinator):
                            ...
                            for msg in messages:
                                if msg.startswith('42') and 'dev_data' in msg:
                                    ...
                                    if isinstance(data_obj, list) and data_obj[0] == "dev_data":
                                        ...
                                        _LOGGER.debug("Received dev_data with %s zone(s)", len(nodes))
    ...
    async def _connect_socketio(self) -> bool:
        ...
        try:
            ...
            async with self.session.post(f"{SOCKETIO_BASE_URL}{SOCKETIO_PATH}?{urlencode(params)}", data=namespace_packet) as resp:
                if resp.status == 200:
            ...
            await self.session.post(f"{SOCKETIO_BASE_URL}{SOCKETIO_PATH}?{urlencode(params)}", data=dev_data_packet)
        ...
        while True:  # Infinite retry loop
            try:
                ...
                if self._reconnect_count > 0:
                    _LOGGER.info("Reconnecting to Socket.IO (attempt %s)...", self._reconnect_count)
                else:
                ...
                while self._connected:
                    try:
                        ...
                        if time_since_update > 300:  # 5 minutes
                            _LOGGER.warning("No updates received for %ss, forcing reconnect...", int(time_since_update))
                            self._connected = False
                            break
                        ...
                        async with self.session.get(url, timeout=aiohttp.ClientTimeout(total=30)) as resp:
                            if resp.status == 200:
                                ...
                                _LOGGER.warning("Socket.IO poll returned HTTP %s, reconnecting...", resp.status)
                                self._consecutive_connection_failures += 1
                                self._connected = False
                                break
